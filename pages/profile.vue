  ...
  <div class="flex flex-row justify-between items-start p-8">
    ...
    <div class="bg-white rounded-lg shadow p-8 w-2/3">
      ...
      <div class="grid grid-cols-2 gap-y-4 gap-x-8 text-base">
        <template v-if="user">      
          <template v-if="user.fullName"><div>Tên nhân sự</div><div>{{ user.fullName }}</div></template>
          <template v-if="user.email"><div>Email</div><div>{{ user.email }}</div></template>
          <template v-if="user.createdAt"><div>Ngày bắt đầu</div><div>{{ formatDate(user.createdAt) }}</div></template>
        </template>
      </div>
    </div>
    ...
    <div class="flex flex-col items-center w-1/3">
      <div class="relative">
        <img :src="user?.avatarUrl || 'https://via.placeholder.com/120'" class="rounded-full w-40 h-40 object-cover border" />
